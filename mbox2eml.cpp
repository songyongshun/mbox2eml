...
/* eml files generated by outlook have a time zone tag, like +0800, which can only be matched by the alternative one */
const std::regex SEPARATOR_R = std::regex("^From .*@.*\\..*( ){1,}[A-Z][a-z]{2} [A-Z][a-z]{2} \\d{1,2} \\d{1,2}:\\d{1,2}:\\d{1,2} \\d{4}$");

/* the alternative one */
const std::regex SEPARATOR_R1 = std::regex("^From .*@.*\\..*( ){1,}[A-Z][a-z]{2} [A-Z][a-z]{2} \\d{1,2} \\d{1,2}:\\d{1,2}:\\d{1,2} \\d{4} \\+\\d{4}$");
  ...
  while (status != ST_END) {
    switch (status) {
      ...
      case ST_SEARCH:
        ...
        if (std::regex_match(line, SEPARATOR_R) || std::regex_match(line, SEPARATOR_R1)) {
          status = ST_SPONGE;
        }
        break;
